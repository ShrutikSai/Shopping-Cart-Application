...
import hashPwd from "../utils/hashedPassword.js";
...
class AuthService {
  ...
  async updateUserAddress(userId, addressId, address) {
    ...
    const { name, street, city, district, ward, phoneNumber, isDefault } =
      address;
  ...
  async updateUser(userId, updateData) {
    ...
    if (username && username !== user.username) {
      const isExistUsername = await this.UserRepository.getUserByUsername(
        username
      );
      if (isExistUsername) {
    ...
    if (phoneNumber && phoneNumber !== user.phoneNumber) {
      const isExistPhone = await this.UserRepository.getUserByPhoneNum(
        phoneNumber
      );
      if (isExistPhone) {
